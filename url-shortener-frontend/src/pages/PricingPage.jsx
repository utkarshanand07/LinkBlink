...
const PricingPage = () => {
    ...
    return (
        <div className="min-h-[calc(100vh-80px)] bg-[#FDFDFD] pb-24 text-slate-900">
            <div className="max-w-7xl mx-auto px-6 lg:px-16 pt-16 sm:pt-24">
                
                {/* Refined Header */}
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="inline-block px-4 py-1.5 mb-6 text-[11px] font-bold tracking-[0.2em] uppercase bg-slate-100 rounded-full text-slate-500">
                        Pricing Plans
                    </motion.div>
                    <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl md:text-6xl font-black tracking-tight mb-6">
                        Scale your links.
                    </motion.h1>
                    <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg text-slate-500 font-medium leading-relaxed">
                ...
                <div className="flex justify-center mb-16">
                    <div className="bg-white/50 backdrop-blur-md p-1.5 rounded-2xl border border-slate-200/60 shadow-sm flex items-center">
                        {['MONTH_1', 'MONTH_3', 'MONTH_6', 'YEAR_1'].map((c) => (
                            <button
                                key={c}
                                onClick={() => setCycle(c)}
                                className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all relative ${cycle === c ? 'bg-white shadow-md text-black border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                {c.replace('MONTH_', '').replace('YEAR_1', '12')}m
                                {c === 'YEAR_1' && <span className="absolute -top-1 -right-1 flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span></span>}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
                    ...
                    {[
                        { title: 'Basic', price: 'Free', sub: 'For individuals.', perks: ['50 Short Links', '30-Day Expiration', 'Standard Analytics'], color: 'bg-white', text: 'text-slate-900', btn: 'bg-slate-50 text-slate-400', forbidden: ['Custom Aliases'] },
                        { title: 'Pro', price: `$${proPricing.monthly}`, sub: 'For professionals.', perks: ['1,000 Short Links', '1 Year Expiration', 'Custom Aliases', 'Standard Analytics'], color: 'bg-slate-900', text: 'text-white', btn: 'bg-white text-black', featured: true, pricing: proPricing },
                        ...
                        <div key={i} className={`${plan.color} ${plan.text} rounded-[2rem] p-8 border border-slate-200/60 shadow-sm flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1 relative overflow-hidden`}>
                            ...
                            <p className={`text-xs font-medium ${plan.featured ? 'text-slate-400' : 'text-slate-500'} mb-8`}>{plan.sub}</p>
                            
                            <div className="mb-10">
                                <span className="text-4xl font-black tracking-tight">{plan.price}</span>
                                {plan.pricing && <span className={`text-xs font-bold ${plan.featured ? 'text-slate-500' : 'text-slate-400'} ml-2 tracking-wide`}>/mo</span>}
                                {plan.pricing && cycle !== 'MONTH_1' && (
                                    <div className="mt-2 text-[10px] font-bold text-green-500 uppercase tracking-widest">
                            ...
                            <ul className="space-y-4 mb-12">
                                {plan.perks.map((p, j) => (
                                    <li key={j} className="flex items-center gap-3 text-sm font-semibold opacity-90">
                                        <FaCheck className={plan.featured ? 'text-yellow-400' : 'text-slate-900'} /> {p}
                                    </li>
                                ))}
                            ...
                            <div className="mt-auto pt-6">
                                <button 
                                    ...
                                    className={`w-full py-4 ${plan.btn} rounded-2xl font-black text-xs uppercase tracking-[0.15em] transition-all active:scale-95 shadow-lg shadow-black/5 disabled:opacity-50 disabled:cursor-not-allowed`}
                                >
